...
gcloud run deploy $SERVICE_NAME `
  ...
  --set-env-vars "NODE_ENV=$nodeEnv,DB_USER=$DB_USER,DB_NAME=$DB_NAME,DB_PORT=$DB_PORT,INSTANCE_CONNECTION_NAME=$INSTANCE_CONNECTION_NAME,GOOGLE_CLIENT_ID=$GOOGLE_CLIENT_ID,ALLOWED_ORIGINS=$ALLOWED_ORIGINS,GCS_BUCKET_DOCUMENTOS=$GCS_BUCKET_DOCUMENTOS,GCS_SIGNED_URL_EXPIRATION_MINUTES=15,MAX_UPLOAD_SIZE_BYTES=52428800" `
